    ...
    <div className="modal-overlay" onClick={onClose} role="dialog" aria-modal="true">
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          <div className={`badge ${trial.phase.toLowerCase().includes('1') ? 'phase-1' : trial.phase.toLowerCase().includes('2') ? 'phase-2' : trial.phase.toLowerCase().includes('3') ? 'phase-3' : 'phase-4'}`}>{trial.phase}</div>
          <p><strong>Location:</strong> {trial.location}</p>
          <p>{trial.description}</p>
        </div>
        <div className="modal-actions">
          <button className="btn" onClick={onClose}>Close</button>
          <button className="btn secondary">Express Interest</button>
        </div>
      </div>
    </div>
  )
    ...
    <div className="container">
      ...
      <main>
        {filtered.length === 0 ? (
          ...
          <div className="grid">
            {filtered.map(t => (
              <TrialCard key={t.id} trial={t} onView={(tr) => setModalTrial(tr)} />
            ))}
          </div>
        )}
      </main>
      ...
      <Modal trial={modalTrial} onClose={() => setModalTrial(null)} />
    </div>
  )
}
